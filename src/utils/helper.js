...
export const calclayout = (unsortLayout = LAYOUT) => {
  console.log('calclayout called with:', unsortLayout);
  console.log('LAYOUT import value:', LAYOUT);
  
  // 有缓存先获取缓存
  if (window.localStorage.getItem(STORAGE_LAYOUT)) {
    console.log('Using localStorage layout');
  ...
  if (!unsortLayout || unsortLayout.length === 0) {
    console.warn('Empty layout detected, returning empty array and 0 count');
    return [unsortLayout || [], 0];
  }
  
  console.log('Layout before sorting:', unsortLayout);
  const layout = unsortLayout.sort(compare("i"));
  console.log('Layout after sorting:', layout);
  
  const len = layout.length;
  const count = parseInt(layout[len - 1].i.split(MARK)[1]) + 1;
  console.log('Returning layout with length:', layout.length, 'and count:', count);
  return [layout, count];
};
...
export const parseMarkdown = itemValue => {
  ...
  html = html ? html : (typeof value === 'string' ? md.render(value) : '');
  
  // Add data-markdown attribute to all HTML elements
  if (html && typeof window !== 'undefined') {
    const tempDiv = document.createElement('div');
    tempDiv.innerHTML = html;
    
    // Add the attribute to all elements
    const allElements = tempDiv.querySelectorAll('*');
    allElements.forEach(element => {
      element.setAttribute('data-markdown', typeof value === 'string' ? value : '');
    });
